			...
			if (tcpCLient && tcpCLient->Socket) {
				...
				case ESocketConnectionState::SCS_Connected:
					...
					if (connectionTimeoutCurrent % 100 == 0) {
						tcpCLient->Send(PlatformCommands::CHECK_IN(udpClientPort, gameName));
					}
					break;
...
void UYawController::DiscoverDevices(int onPort)
{
	...
	udpClient->SendBroadcast(onPort, PlatformCommands::DEVICE_DISCOVERY);
}
...
void UYawController::SetYawLimit(int yawLimitI)
{
    ...
    {
		tcpCLient->Send(PlatformCommands::SET_YAW_LIMIT(yawLimitI));
    }
}
...
void UYawController::SetTiltLimits(int pitchForwardLimit, int pitchBackwardLimit, int rollLimit)
{
    ...
    if (state != ControllerState::Initial && state != ControllerState::Disconnecting)
    {
		tcpCLient->Send(PlatformCommands::SET_TILT_LIMITS(pitchForwardLimit, pitchBackwardLimit, rollLimit));
    }
}
...
void UYawController::StartDevice()
{
    if (state == ControllerState::Connected)
    {
		...
		*/
        //Send START command
		tcpCLient->Send(PlatformCommands::START);
...
void UYawController::StopDevice()
{
    if (state == ControllerState::Started)
    {
		...
		*/
        //Send STOP command
		tcpCLient->Send(PlatformCommands::STOP);
...
void UYawController::DisconnectFromDevice()
{
    ...
    if (state != ControllerState::Initial)
    {
		...
		*/
        //Send EXIT command
		tcpCLient->Send(PlatformCommands::EXIT);
...
void UYawController::DidRecieveTCPMessage(TArray<uint8> data)
{
    ...
    switch (commandId)
    {
        ...
        case CommandIds::SET_YAW_LIMIT:
            if (data.Num() == 5) {
                int limit = PlatformCommands::ByteArrayToInt(data, 1);
				this->yawLimit = limit;
        ...
        case CommandIds::SET_TILT_LIMITS:
            if (data.Num() == 13)
            {
                int pitchForwardLimit = PlatformCommands::ByteArrayToInt(data, 1);
                int pitchBackwardLimit = PlatformCommands::ByteArrayToInt(data, 5);
                int rollLimit = PlatformCommands::ByteArrayToInt(data, 9);
...
void UYawController::SendRotation(FVector rotation)
{
	...
	udpClient->Send(PlatformCommands::SET_POSITION(y, x, z, CurrentBuzzer.RightBuzzer, CurrentBuzzer.CenterBuzzer, CurrentBuzzer.LeftBuzzer, CurrentBuzzer.BuzzerHz));
}
