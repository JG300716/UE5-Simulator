...
const TArray<uint8> PlatformCommands::START({ CommandIds::START });
const TArray<uint8> PlatformCommands::STOP ({ CommandIds::STOP  });
const TArray<uint8> PlatformCommands::EXIT ({ CommandIds::EXIT  });

const TArray<uint8> PlatformCommands::DEVICE_DISCOVERY(FStringToBytes("YAW_CALLING"));



//example: "Y[000.00]P[359.99]R[180.00]"; - there is no 360.00, just 000.00
TArray<uint8> PlatformCommands::SET_POSITION(float yaw, float pitch, float roll, int rightBuzzer, int centerBuzzer, int leftBuzzer, int buzzerHz)
{
...
TArray<uint8> PlatformCommands::CHECK_IN(int udpListeningPort, FString gameName)
{
...
TArray<uint8> PlatformCommands::SET_TILT_LIMITS(int pitchFrontMax, int pitchBackMax, int rollMax)
{
	TArray<uint8> message;
...
TArray<uint8> PlatformCommands::SET_YAW_LIMIT(int yawMax)
{
	TArray<uint8> message;
...
FString PlatformCommands::FormatRotation(float f)
{
...
FString PlatformCommands::FormatInt(int i)
{
	FString s = FString::FromInt(FMath::Clamp(i, 0, 100));
	return s;
}

TArray<uint8> PlatformCommands::IntToByteArray(int intValue)
{
...
TArray<uint8> PlatformCommands::FloatToByteArray(float floatValue)
{
...
int PlatformCommands::ByteArrayToInt(TArray<uint8> intBytes, int startIndex)
{
...
float PlatformCommands::ByteArrayToFloat(TArray<uint8> floatBytes, int startIndex)
{
...
TArray<uint8> PlatformCommands::FStringToBytes(FString Message)
{
